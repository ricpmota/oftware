...
const EMAIL_DOC_ID = 'bem_vindo_nutri_bem_vindo_nutricionista';
const EMAIL_TIPO = 'bem_vindo_nutri_bem_vindo_nutricionista';
...
export async function POST(request: NextRequest) {
  try {
    const { nutricionistaId } = await request.json();
    if (!nutricionistaId) return NextResponse.json({ error: 'nutricionistaId é obrigatório' }, { status: 400 });
    ...
    const nome = nutri.nome || nutriEmail.split('@')[0] || 'Nutricionista';
    ...
    await db.collection('email_envios').add({ emailTipo: EMAIL_TIPO, destinatarioEmail: nutriEmail, assunto, enviadoEm: new Date(), status: envioSucesso ? 'enviado' : 'falhou', erro: erroEnvio || null, tipo: 'automatico', nutricionistaId });
    if (!envioSucesso) return NextResponse.json({ error: 'Erro ao enviar e-mail', details: erroEnvio }, { status: 500 });
    return NextResponse.json({ success: true, message: 'E-mail enviado com sucesso' });
  } catch (error) {
    console.error('send-email-bem-vindo-nutri:', error);
