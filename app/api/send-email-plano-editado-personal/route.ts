...
const EMAIL_DOC_ID = 'em_tratamento_personal_plano_editado';
const EMAIL_TIPO = 'em_tratamento_personal_plano_editado';
...
export async function POST(request: NextRequest) {
  try {
    const { pacienteId, personalId } = await request.json();
    if (!pacienteId || !personalId) return NextResponse.json({ error: 'pacienteId e personalId são obrigatórios' }, { status: 400 });
    ...
    const personalNome = personalDoc.exists ? (personalDoc.data()?.nome || 'Personal') : 'Personal';
    ...
    let assunto = t.assunto || 'Seu plano de treinos foi atualizado';
    let html = (t.corpoHtml || '').replace(/\{nome\}/g, nome).replace(/\{personal\}/g, personalNome);
    assunto = assunto.replace(/\{nome\}/g, nome).replace(/\{personal\}/g, personalNome);
    ...
    await db.collection('email_envios').add({ emailTipo: EMAIL_TIPO, destinatarioEmail: pacienteEmail, assunto, enviadoEm: new Date(), status: envioSucesso ? 'enviado' : 'falhou', erro: erroEnvio || null, tipo: 'automatico', pacienteId, personalId });
    if (!envioSucesso) return NextResponse.json({ error: 'Erro ao enviar e-mail', details: erroEnvio }, { status: 500 });
    return NextResponse.json({ success: true, message: 'E-mail enviado com sucesso' });
  } catch (error) {
    console.error('send-email-plano-editado-personal:', error);
