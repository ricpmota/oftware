...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!paciente?.planoTerapeutico) {
      return NextResponse.json(
        { error: 'Paciente não tem plano de tratamento' },
        { status: 400 }
      );
    }
    ...
    const emailDoc = await db.collection('emails').doc('em_tratamento_plano_editado').get();
    if (!emailDoc.exists) {
      return NextResponse.json(
    ...
    const assunto = emailTemplate?.assunto || 'Seu plano de tratamento foi atualizado';
    let html = emailTemplate?.corpoHtml || '';

    // 4. Substituir variáveis
    const dataInicio = planoTerapeutico.startDate?.toDate?.() || 
                      (planoTerapeutico.startDate instanceof Date ? planoTerapeutico.startDate : new Date());
    const semanas = planoTerapeutico.numeroSemanasTratamento || 18;
  ...
  ${html}
    ...
    try {
      if (isZeptoMailConfigured()) {
        ...
        else console.log('✅ E-mail de plano editado enviado:', sent.messageId);
      } else {
    ...
    await enviosCollection.add({
      ...
      emailTipo: 'em_tratamento_plano_editado',
      assunto,
      enviadoEm: new Date(),
    ...
    return NextResponse.json({
      success: true,
      message: 'E-mail de plano editado enviado com sucesso',
    });
  } catch (error) {
    console.error('Erro ao enviar e-mail de plano editado:', error);
    return NextResponse.json(
