 ...
 * Busca exercícios por equipamento na ExerciseDB.
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!equipment) {
      return NextResponse.json(
        { error: 'Parâmetro "equipment" é obrigatório' },
        { status: 400 }
      );
    }

    const res = await fetch(
      `${base}/exercises/equipment/${encodeURIComponent(equipment)}?limit=${limit}&offset=${offset}`,
      {
        method: 'GET',
        headers: {
          'X-RapidAPI-Key': key,
