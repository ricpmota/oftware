  ...
  try {
    ...
    const originalPdfUrl =
      typeof body.originalPdfUrl === 'string' && body.originalPdfUrl.trim()
        ...
        : `sandbox://exam_request/${patientId}/${Date.now()}.pdf`;
    ...
    console.error('[signature/sandbox/exam-request]', error);
    const message =
      error instanceof Error ? error.message : 'Erro ao assinar PDF de exames no sandbox.';
