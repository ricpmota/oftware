...
type Body = {
  ...
  prescriptionPrintType?: 'simple' | 'controle_especial';
};
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const printType =
      body.prescriptionPrintType === 'controle_especial' ? 'controle_especial' : 'simple';

    const result = await submitBryPrescriptionSignature({
    ...
    console.error('[signature/bry/prescription]', logPayload);
    const message =
      error instanceof Error ? error.message : 'Erro ao enviar prescrição para assinatura BRy Cloud.';
