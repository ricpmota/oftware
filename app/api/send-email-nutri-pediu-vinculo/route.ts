...
const EMAIL_DOC_ID = 'nutri_pediu_vinculo_aviso_medico';
const EMAIL_TIPO = 'nutri_pediu_vinculo_aviso_medico';
...
export async function POST(request: NextRequest) {
  try {
    const { medicoId, nutricionistaId, pacienteId } = await request.json();
    if (!medicoId || !nutricionistaId) return NextResponse.json({ error: 'medicoId e nutricionistaId são obrigatórios' }, { status: 400 });
    ...
    let nutricionistaNome = 'Nutricionista';
    const nutriDoc = await db.collection('nutricionistas').doc(nutricionistaId).get();
    if (nutriDoc.exists) nutricionistaNome = nutriDoc.data()?.nome || nutricionistaNome;
    else {
      const byUserId = await db.collection('nutricionistas').where('userId', '==', nutricionistaId).limit(1).get();
      if (!byUserId.empty) nutricionistaNome = byUserId.docs[0].data()?.nome || nutricionistaNome;
    }
    ...
    let assunto = t.assunto || 'Nutricionista solicitou vínculo';
    let html = (t.corpoHtml || '').replace(/\{medico\}/g, medicoDisplay).replace(/\{nutricionista\}/g, nutricionistaNome).replace(/\{nome\}/g, nome);
    assunto = assunto.replace(/\{medico\}/g, medicoDisplay).replace(/\{nutricionista\}/g, nutricionistaNome).replace(/\{nome\}/g, nome);
    ...
    await db.collection('email_envios').add({ emailTipo: EMAIL_TIPO, destinatarioEmail: medicoEmail, assunto, enviadoEm: new Date(), status: envioSucesso ? 'enviado' : 'falhou', erro: erroEnvio || null, tipo: 'automatico', medicoId, nutricionistaId, pacienteId });
    if (!envioSucesso) return NextResponse.json({ error: 'Erro ao enviar e-mail', details: erroEnvio }, { status: 500 });
    return NextResponse.json({ success: true, message: 'E-mail enviado com sucesso' });
  } catch (error) {
    console.error('send-email-nutri-pediu-vinculo:', error);
