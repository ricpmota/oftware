...
export default function MetaAdminGeralLeadsWhiteLabelRedirectPage() {
  const router = useRouter();

  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (firebaseUser) => {
      if (!firebaseUser) {
        router.replace(metaAdminGeralAcessarUrl('/metaadmingeral?menu=leads-whitelabel'));
        return;
      }
      ...
      router.replace('/metaadmingeral?menu=leads-whitelabel');
    });
