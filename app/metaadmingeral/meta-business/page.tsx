...
export default function MetaAdminGeralMetaBusinessRedirectPage() {
  const router = useRouter();

  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (firebaseUser) => {
      if (!firebaseUser) {
        router.replace(metaAdminGeralAcessarUrl('/metaadmingeral?menu=meta-business'));
        return;
      }
      ...
      router.replace('/metaadmingeral?menu=meta-business');
    });
